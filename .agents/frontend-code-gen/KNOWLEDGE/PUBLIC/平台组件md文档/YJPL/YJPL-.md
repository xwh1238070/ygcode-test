...
## 介绍
YJPL是远光九天统一界面描述语言，全面兼容VUE语法，具有以下优势：  
1. 易用：只要会HTML、CSS、JavaScript，即可开始使用
2. 灵活：模板继承与UI解耦，实渐进式框架，更加灵活。
3. 高效：真正的按需加载，精确到方法。超快虚拟DOM和最省心的优化。

为了更高效的支持开发，YJPL支持提供能力：
1. 逻辑与视图分离  
采用了最新的MVVM模式，数据双向绑定， 业务逻辑与视图分离，各自内容变更互不影响。
2. 前后端分离  
前端开发和后端开发可以相互独立，互不影响，独立运行。
3. 页面模块化  
每个页面都可以是一个模块、复用率高、减少开发资源、 提高开发效率。
4. 采用ES编码
采用最新的ES编码、开发更灵活、更便捷。
5. 单元测试  
支持对象、界面进行单元测试、自动化测试、提高代码稳定性。
6. 规范检查  
自动对代码的规范进行检查、规范开发人员的编码风格、 提高维护性。


## 开发环境

### 1. 客户端软件
1. IDE及版本：VSCODE V1.16 + 
2. 中间件及版本：NODEJS  V14+
3. 浏览器及版本：谷歌V50+、火狐30+、IE11+等

[相关工具下载地址](https://pan.ygsoft.com/l/SFhnmf)
...
### 3. 下载脚手架
执行命令：npm install tlead-cli –g

### 4. 使用脚手架创建项目
执行命令：tlead create [项目名称] 
1.	下载空白工程：tlead create prj 
2.	下载样例工程：tlead create demo -d
3.	下载样例工程并运行：tlead create demo –d -r
...
### 3. 文件引用
- **路径说明**
1. 以@开头:   在yjpl.config.js中配置了 @代表目录src
2. 以./开头： 在项目中可以用./代表相对路径，如目录结构

        pages
            |- a.ts
            |- b.ts

        b.ts 要引用a.ts 就可以用 import a from './a'    （注意： 无后缀名）

3. 无路径，直接一个字符串的，如 import 'yjpl-ui'，这种类型都是在node_modules内
### 4. 路由配置
1.  路由配置在router/index.ts文中
``` js
import Yjpl from 'yjpl';
import YjplRouter, { RouteConfig } from 'yjpl-router';
import Index from '@/pages/index.yjpl';
Yjpl.use(YjplRouter);
...
- 配置在yjpl.config.ts文中找到devServer/proxy节点, 若没有则在根节点下面创建一个。
``` js
module.exports = {
  ...
  devServer: {
    proxy: {
      '/necp/mapp/4a': {
        target: 'http://10.52.3.124:8087',      // 代理的服务器地址
...
1.  打开src/config/config.ts文件
2.  配置好contextPath属性
3.  配置securityCheck为true


## 数据访问
### 远程调用
天鹂提供统一的服务端请求接口， 内部集成安全加密、签名等安全机制。可通过 页面对象的tleadService 属性调用。其中包括两个异步方法：
1. doGet(url, params)
2. doPost(url, params)
...
const yjplServer = new YJService();

async function getData(){
  let res;
  try{
    res = await yjplServer.doGet(url, param);
    // 
    res = await yjplServer.doPost(url, param);
  } catch(e){
    console.error(e)
  }
  return res;
...
``` 
### mock数据
到mock目录下的index.ts 添加mock设置，在doGet中拦截doGet请求，在doPost中拦截doPost请求。
``` js   
export default {
    doGet: {
        '/model/metaModel': {
...
## 动态扩展
利用动态扩展机制，可以在不直接修改原页面的情况下为页面增加或者替换已有能力。  
1. 对原页面的视图模板进行新增、删除、修改。
2. 新增或者覆盖视图逻辑的数据、事件、方法等。
3. 新增或者覆盖业务逻辑的数据、事件、方法等。
...
其中，extend为扩展的根文件夹，名称为任意有意义的值。
index.yjpl为固定名称的视图文件，用于扩展页面的视图模板。

## 行为特性
提供行为特性功能，支持在页面动态执行自定义行为。  
### 目录结构
      src源码目录  
      |- action  
      |-   |- index.ts    导出要注册的行为信息  
...
### 定义
``` js
// src/action/xxxAction.ts
class LogAction extends YJAction {
  /**
   * 执行
   * @param yjpl  yjpl页面对象
   * @param option 自定义参数
...
   Yjpl.prototype.tleadAction.register(action.name, action.action);
});
…
```
### 执行
...
export default class YwdjView extends YJPage {
  created(){
    // 在任意yjpl页面中执行行为：
    this.tleadAction.trigger('log', this, {});
  }
}
</script>
```
## 动态路由
动态路由：同一地址根据查询条件不同，路由到不同的页面。
例如：  
1. http://localhost:8080/dyRoute?pageType=add 路由到填单页面：
2. http://localhost:8080/dyRoute?pageType=list 路由到列表页面：

### 配置
1、在src/router/index.ts 配置填单和列表页面的路由信息。  
``` js
  ...
  {
    path: '/add-page',
...
const yjUtils = new YJUtils();
export default class YwdjView extends YJPage {
  constructor() {
    super();
    this.$route.query && yjUtils.dyLoadRoute(this.$route.query.pageType, {
      'add': 'add-page',
      'list': 'list-page'
    }, this.$router);
  }
...
### 安装
#### 直接下载 / CDN 引用
[https://unpkg.com/vuex](https://unpkg.com/vuex@3.6.2/dist/vuex.js)  
...
```
上面插件会默认启用。在发布阶段，你需要使用 webpack 的 DefinePlugin或者是 Browserify 的 envify 使 process.env.NODE_ENV !== 'production' 为 false。
...
### 热重载
使用 webpack 的 [Hot Module Replacement API](https://webpack.js.org/guides/hot-module-replacement/)，Vuex 支持在开发过程中热重载 mutation、module、action 和 getter。你也可以在 Browserify 中使用 [browserify-hmr](https://github.com/Macil/browserify-hmr)插件。
