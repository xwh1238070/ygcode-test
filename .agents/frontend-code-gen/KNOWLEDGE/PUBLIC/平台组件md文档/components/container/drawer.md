...
```html
<template>
  <tl-radio-group v-model="direction">
    <tl-radio label="ltr">从左往右开</tl-radio>
    <tl-radio label="rtl">从右往左开</tl-radio>
    <tl-radio label="ttb">从上往下开</tl-radio>
    <tl-radio label="btt">从下往上开</tl-radio>
  </tl-radio-group>

  <tl-button @click="drawer = true" type="primary" style="margin-left: 16px;"> 点我打开 </tl-button>

  <tl-drawer title="我是标题" :visible.sync="drawer" :direction="direction" :before-close="handleClose">
    <span>我来啦!</span>
  </tl-drawer>
</template>
...
<template>
  <tl-button @click="drawer = true" type="primary" style="margin-left: 16px;"> 点我打开 </tl-button>
  <tl-drawer title="我是标题" :visible.sync="drawer" :with-header="false">
    <span>我来啦!</span>
  </tl-drawer>
</template>
...
```html
<template>
  <tl-button type="text" @click="table = true">打开嵌套表格的 Drawer</tl-button>
  <tl-button type="text" @click="dialog = true">打开嵌套 Form 的 Drawer</tl-button>
  <tl-drawer title="我嵌套了表格!" :visible.sync="table" direction="rtl" size="50%">
  ...
  >
    <div class="demo-drawer__content">
      ...
      <div class="demo-drawer__footer">
        <tl-button @click="cancelForm">取 消</tl-button>
        <tl-button type="primary" @click="$refs.drawer.closeDrawer()" :loading="loading"
        >{{ loading ? '提交中 ...' : '确 定' }}</tl-button>
      </div>
    </div>
  </tl-drawer>
</template>
...
<template>
  <tl-button @click="drawer = true" type="primary" style="margin-left: 16px;"> 点我打开 </tl-button>
  <tl-drawer title="我是外面的 Drawer" :visible.sync="drawer" size="50%">
    <div>
      <tl-button @click="innerDrawer = true">打开里面的!</tl-button>
      <tl-drawer title="我是里面的" :append-to-body="true" :before-close="handleClose" :visible.sync="innerDrawer">
        <p>_(:зゝ∠)_</p>
      </tl-drawer>
    </div>
  </tl-drawer>
...
:::demo 如果需要配置拖拽的 `Drawer`，可以配置 `draggable` 属性，只有在 `direction` 为 `rtl` 和 `ltr` 时生效。

```html
<template>
  <tl-button @click="drawer = true" type="primary" style="margin-left: 16px;"> 点我打开 </tl-button>
  <tl-drawer title="我是标题" draggable :visible.sync="drawer">
    <span>我来啦!</span>
  </tl-drawer>
</template>
...
| max-size | Drawer 最大宽度，当设置 drag 后生效。 | number / string | - | '30%' |
| drag     | Drawer 是否可通过拖拽改变宽度，只有在 direction 为 rtl 和 ltr 时生效。   | boolean   | — | false |
...
| drag-begin | Drawer 开始拖拽时的回调 | — |
| dragging | Drawer 拖拽中的回调 | — |
| drag-end | Drawer 拖拽结束的回调 | — |
