...
<script>
  export default {
    ...
    mounted() {
      ...
      this.grid.bind('onCellDraw_STOP', function (dataSet, fieldName, col) {
        if (this.getValue('dxmc') == 2) {
          //隐藏开关
          col.checkBox.onoff.hide();
          //如果是复选框(checkBox) 则直接用 col.checkBox.hide()/show()
        } else {
          col.checkBox.onoff.show();
        }
      });

      this.grid.bind('onCheckChanged', function (node, state) {
        if (state === true) {
          alert(this.getValue('dxmc', node) + 'on');
        } else {
          alert('off');
        }
      });
