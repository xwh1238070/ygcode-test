...
```html
<template>
  <tl-button type="text" @click="open">点击打开 Message Box</tl-button>
</template>
...
### 居中图标消息
内容支持居中布局，添加图标消息，必须搭配`type`字段

:::demo 传递 `iconText` 和 `type` 即可开启居中图标消息提示

```html
<template>
  <tl-button type="text" @click="open">点击打开成功</tl-button>
  <tl-button type="text" @click="open1">点击打开失败</tl-button>
  <tl-button type="text" @click="open2">点击打开警告</tl-button>
</template>
...
如果你完整引入了 `yjpl-ui`，它会为 Yjpl.prototype 添加如下全局方法：$msgbox, $alert, $confirm 和 $prompt。因此在 Yjpl instance 中可以采用本页面中的方式调用 `MessageBox`。调用参数为：
- `$msgbox(options)`
...
```javascript
import { MessageBox } from 'yjpl-ui';
```
...
| iconText | 图标文字（必须搭配type使用） | string | — | '' |
