...
<template>
  <div class="block">
    ...
    <tl-rate v-model="value1"></tl-rate>
  </div>
  <div class="block">
...
:::demo 设置`icon-classes`属性可以自定义不同分段的图标。若传入数组，共有 3 个元素，为 3 个分段所对应的类名；若传入对象，可自定义分段，键名为分段的界限值，键值为对应的类名。本例还使用`void-icon-class`
...
<script>
  export default {
    data() {
      return {
        value: null,
        iconClasses: ['yj-p-countersign', 'yj-p-wendang', 'yj-p-quanqing'] // 等同于 { 2: 'icon-rate-face-1', 4: { value: 'icon-rate-face-2', excluded: true }, 5: 'icon-rate-face-3' }
      }
    }
  };
</script>
