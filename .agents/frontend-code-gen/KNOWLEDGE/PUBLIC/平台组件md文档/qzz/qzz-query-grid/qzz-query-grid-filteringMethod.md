...
<script>
  export default {
    ...
    mounted() {
      ...
      var _this = this;
      this.subgrid.value(this.maingriddata);
      //过滤事件
      this.subgrid.dataSet.bind('onFilter', function (dataSet, state) {
        if (dataSet.getValue('TJBM') !== _this.maingrid.getValue('TJBM')) {
          //需要过滤的数据返回true
          return true;
        } else {
          //不需要过滤的数据返回false
      ...
      this.maingrid.bind('onRowChanged', function () {
        _this.subgrid.setFiltered(true);
      });

      this.subgrid.setFiltered(true);
    }
  };
</script>
```
